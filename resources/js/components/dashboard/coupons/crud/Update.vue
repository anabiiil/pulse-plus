...
useHead({ title: 'Update Coupon' });

const router = useRouter();
const route = useRoute();
const { update, getCoupon, coupon } = useCoupons();
...
const loadCoupon = async () => {
    try {
        ...
        if (coupon.value) {
            ...
            formData.type = coupon.value.type || 'percentage';
            formData.value = coupon.value.value ?? 0;
            formData.starts_at = (coupon.value as any).starts_at || '';
            formData.expires_at = (coupon.value as any).expires_at || '';
            formData.status = coupon.value.status === 1 || coupon.value.status === 'active' || coupon.value.status === true;
        }
    } catch (error) {
...
const handleSubmit = async () => {
    ...
    try {
        await update(couponId.value, { ...formData, status: formData.status ? 1 : 0 });
        window.showSuccessToast?.('Coupon updated successfully');
        await router.push('/dash/coupons');
    } catch (error: any) {
        const apiErrors = error?.response?.data?.errors || {};
        Object.assign(errors, apiErrors);
        window.showErrorToast?.(error?.response?.data?.message || 'Failed to update coupon');
    } finally {
        loading.value = false;
    }
};

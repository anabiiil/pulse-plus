                            ...
                            <div class="row">
                                <div class="col-12 my-4">
                                    <div class="alert alert-danger d-flex align-items-center" role="alert">
                                        ...
                                        <div>
                                            Are you sure you want to delete <strong>{{ governorate?.name?.en }}</strong>? This action cannot be undone.
                                        </div>
                                    </div>
                                </div>
...
const { delete: deleteGovernorate, getGovernorate: fetchGovernorate, governorate } = useGovernorates();
...
const loadGovernorate = async () => {
    try {
        ...
        if (!governorate) {
            showErrorToast('Governorate not found');
            await router.push('/dash/governorate');
        }
    } catch (error: any) {
        const errorMsg = error?.response?.data?.message || 'Failed to load governorate';
...
const handleDelete = async () => {
    ...
    try {
        ...
        const errorMsg = error?.response?.data?.message || 'Failed to delete governorate';
        showErrorToast(errorMsg);
    } finally {

                            ...
                            <div class="row">
                                <div class="col-12 my-4">
                                    <div class="alert alert-danger d-flex align-items-center" role="alert">
                                        ...
                                        <div>
                                            Are you sure you want to delete <strong>{{ slider?.title?.en }}</strong>? This action cannot be undone.
                                        </div>
                                    </div>
                                </div>
...
const { delete: deleteSlider, getSlider: fetchSlider, slider } = useSliders();
...
const loadSlider = async () => {
    try {
        ...
        if (!slider.value) {
            window.showErrorToast('Slider not found');
...
const handleDelete = async () => {
    ...
    try {
        await deleteSlider(sliderId.value);
        await router.push('/dash/sliders');
    } catch (error: any) {
        const errorMsg = error?.response?.data?.message || 'Failed to delete slider';
        window.showErrorToast(errorMsg);
    } finally {
